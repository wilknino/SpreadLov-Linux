...
export default function ChatWindow({ currentUser, selectedUser, onToggleUserInfo }: ChatWindowProps) {
  ...
  useEffect(() => {
    const fetchConsentStatus = async () => {
      ...
      console.log(`[ChatWindow] Fetching consent status for user ${selectedUser.id}`);
      try {
        ...
        if (response.ok) {
          ...
          console.log('[ChatWindow] Fetched consent status:', data);
  ...
  useEffect(() => {
    const handleConsentRequest = (event: CustomEvent) => {
      ...
      console.log('[ChatWindow] Received consent request event:', { consent, requester, selectedUserId: selectedUser?.id });
      
      // Check if this consent request is for the current conversation
      if (selectedUser && requester.id === selectedUser.id) {
        console.log('[ChatWindow] Setting consent request for selected user');
