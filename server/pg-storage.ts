...
export class PgStorage implements IStorage {
  ...
  async createAccountDeletionCode(email: string, code: string, expiresAt: Date): Promise<void> {
    const { accountDeletionCodes } = await import("@shared/schema");
    await this.db
  ...
  async deleteAllUserData(email: string): Promise<void> {
    ...
    await this.db.delete(messages).where(eq(messages.senderId, user.id));
    await this.db.delete(notifications).where(or(eq(notifications.userId, user.id), eq(notifications.fromUserId, user.id)));
    await this.db.delete(chatConsents).where(or(eq(chatConsents.requesterId, user.id), eq(chatConsents.responderId, user.id)));
    await this.db.delete(profileLikes).where(or(eq(profileLikes.likerId, user.id), eq(profileLikes.likedUserId, user.id)));
    await this.db.delete(conversations).where(or(eq(conversations.participant1Id, user.id), eq(conversations.participant2Id, user.id)));
  ...
  async cleanupExpiredDeletionCodes(): Promise<void> {
    ...
    await this.db
      ...
      .where(sql`${accountDeletionCodes.expiresAt} < NOW()`);
  }
}