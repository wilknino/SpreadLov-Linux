...
app.get("/api/users/:userId", async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    // ✅ FIX: Only create profile view notification if one doesn't exist in the last 5 minutes
    try {
      const existingNotifications = await storage.getUserNotifications(userId);
      const fiveMinutesAgo = new Date(Date.now() - 1 * 60 * 1000); // 5 minutes cooldown
      
      const recentProfileView = existingNotifications.find(notification => 
        notification.type === 'profile_view' &&
        notification.fromUserId === req.user!.id &&
        notification.createdAt && new Date(notification.createdAt) > fiveMinutesAgo
      );
      
      if (!recentProfileView) {
        ...
        if (fromUser) {
          ...
          // Also send a dedicated event for notification counter update
          sendToUser(userId, {
        ...
        console.log(`⏭️ Skipping duplicate profile view notification (already sent within 5 minutes)`);
      }
        ...
        switch (message.type) {
          ...
          case 'sendMessage':
            ...
            const newMessage = await storage.createMessage({
              ...
              // ✅ ADD THIS: Create notification for message received
