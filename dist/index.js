...
async function handleNotificationWebhook(req, res) {
  try {
    ...
    if (!success) {
      ...
      log(`\u{1F504} Cleared invalid FCM token for user ${recipientUser.id}`);
    }
    log(`\u{1F4EC} Notification webhook processed: ${notification.type} for user ${notification.userId}`);
    return res.status(200).json({
    ...
    client.on("notification", async (msg) => {
      if (msg.channel === "new_notification" && msg.payload) {
        try {
          ...
          if (!success) {
            await db.update(users).set({ fcmToken: null }).where(eq3(users.id, recipientUser.id));
            log(`\u{1F504} Cleared invalid FCM token for user ${recipientUser.id}`);
          }
        } catch (error) {
