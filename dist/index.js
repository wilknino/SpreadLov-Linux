...
import { eq, and, or, desc, sql as sql2, ne } from "drizzle-orm";
  ...
  app2.get("/api/users/:userId", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const fiveMinutesAgo = new Date(Date.now() - 1 * 60 * 1e3);
        const recentProfileView = existingNotifications.find(
          (notification) => notification.type === "profile_view" && notification.fromUserId === req.user.id && new Date(notification.createdAt) > fiveMinutesAgo
        );
